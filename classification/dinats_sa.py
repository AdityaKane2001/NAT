...
class MHSARPBFlashAttn(nn.Module):
    def __init__(self, dim, input_size, num_heads,
                 qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = F.scaled_dot_product_attention(
            q, k, v, attn_mask=attn_bias, dropout_p=self.attn_drop
        )
...
class DiNAT_s(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.input_sizes = [img_size // 4, img_size // 8,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                input_size=self.input_sizes[i_layer],
...
@register_model
def dinat_s_sa_tiny(pretrained=False, use_flash_attn=True, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 6, 2],
        num_heads=[3, 6, 12, 24],
        embed_dim=96,
...
@register_model
def dinat_s_sa_small(pretrained=False, use_flash_attn=True, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        num_heads=[3, 6, 12, 24],
        embed_dim=96,
...
@register_model
def dinat_s_sa_base(pretrained=False, use_flash_attn=True, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        num_heads=[4, 8, 16, 32],
        embed_dim=128,
...
@register_model
def dinat_s_sa_large(pretrained=False, use_flash_attn=True, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        num_heads=[4, 8, 16, 32],
        embed_dim=192,
...
@register_model
def dinat_s_sa_large_384(pretrained=False, use_flash_attn=True, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        num_heads=[4, 8, 16, 32],
        embed_dim=192,
...
@register_model
def dinat_s_sa_large_21k(pretrained=False, use_flash_attn=True, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        num_heads=[4, 8, 16, 32],
        embed_dim=192,

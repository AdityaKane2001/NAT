...
@register_model
def dinat_s_no_rpb_tiny(pretrained=False, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 6, 2],
        rpb=False,
        num_heads=[3, 6, 12, 24],
    ...
    if pretrained:
        url = model_urls["dinat_s_no_rpb_tiny_1k"]
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint)
    return model
...
@register_model
def dinat_s_no_rpb_small(pretrained=False, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        rpb=False,
        num_heads=[3, 6, 12, 24],
    ...
    if pretrained:
        url = model_urls["dinat_s_no_rpb_small_1k"]
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint)
    return model
...
@register_model
def dinat_s_no_rpb_base(pretrained=False, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        rpb=False,
        num_heads=[4, 8, 16, 32],
    ...
    if pretrained:
        url = model_urls["dinat_s_no_rpb_base_1k"]
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint)
    return model
...
@register_model
def dinat_s_no_rpb_large(pretrained=False, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        rpb=False,
        num_heads=[4, 8, 16, 32],
    ...
    if pretrained:
        url = model_urls["dinat_s_no_rpb_large_1k"]
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint)
    return model
...
@register_model
def dinat_s_no_rpb_large_384(pretrained=False, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        rpb=False,
        num_heads=[4, 8, 16, 32],
    ...
    if pretrained:
        url = model_urls["dinat_s_no_rpb_large_1k_384"]
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint)
    return model
...
@register_model
def dinat_s_no_rpb_large_21k(pretrained=False, **kwargs):
    model = DiNAT_s(
        depths=[2, 2, 18, 2],
        rpb=False,
        num_heads=[4, 8, 16, 32],
    ...
    if pretrained:
        url = model_urls["dinat_s_no_rpb_large_21k"]
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint)
    return model
